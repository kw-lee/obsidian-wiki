...
- **File management**: explorer toolbar actions (new note/folder, sort, expand/collapse, reveal current note), independent explorer scrolling on desktop/mobile, drag-and-drop moves, optional wikilink rewriting on move, and refresh-free optimistic UI updates after moves.
...
- **Session security**: access token in session storage + refresh token in `httpOnly` cookie
- **Sync**: Git or WebDAV
...
- `README.md` is the default user-facing README and stays in Korean.
- `README.en.md` is the English user-facing README.
...
This program was developed in whole or in part with the assistance of large language models (LLMs). The authors and contributors disclaim responsibility for bugs, incorrect behavior, security issues, data loss, or any other problems that may arise from using, modifying, or deploying this software. You are solely responsible for reviewing the code, validating the behavior, and determining whether it is safe and suitable for your environment.

## License

The project source code and project-authored docs are licensed under the [MIT License](LICENSE) (`MIT`).

Bundled third-party font assets under [`frontend/static/fonts/`](frontend/static/fonts/) are not relicensed under the repository root `MIT` notice and remain under their upstream terms. See [`frontend/static/fonts/README.md`](frontend/static/fonts/README.md).
