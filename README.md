...
소스를 서버에서 직접 빌드하는 방식은 기존처럼 `docker-compose.yml` 을 그대로 쓰면 됩니다.

Docker Hub 없이 GitHub 기반 이미지 배포를 쓰려면 GitHub Container Registry (`ghcr.io`) 를 사용하세요. 이 저장소에는 [`.github/workflows/publish-ghcr.yml`](.github/workflows/publish-ghcr.yml) 워크플로와 [`docker-compose.ghcr.yml`](docker-compose.ghcr.yml) 오버라이드가 포함되어 있어, GitHub Actions 가 `backend` / `frontend` 이미지를 GHCR 로 푸시하고 서버는 아래처럼 바로 가져와 실행할 수 있습니다.

```bash
docker compose -f docker-compose.yml -f docker-compose.ghcr.yml up -d
```

실행 전에 `.env` 에 `BACKEND_IMAGE`, `FRONTEND_IMAGE` 를 원하는 GHCR 태그로 넣어두면, Postgres / Redis / nginx 는 기존 compose 구성을 그대로 사용하면서 앱 서비스만 `build:` 대신 `image:` 기반으로 바뀝니다.
